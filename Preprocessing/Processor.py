...
key = 'AIzaSyChY_02V3gWYfzI0O0qBkjozaBSsuh25'
...
def lookupContinent(country):
    ...
    for elements in countries:
        if(country == elements['name']):

            return elements['continent']
...
with open('flickr100k-photos.txt') as f:
    lines = [line.rstrip('\n') for line in open('flickr100k-photos.txt')]
    for line in lines:
...
temp = [parseElem(elements) for elements in jsonList]

file = open('updatedJSONV2.json', 'w')
#file.write("[")

for i in range (0, len(temp)):
    tempVar = json.dumps(temp[i])
    if i != (len(temp)-1):
        file.write(tempVar + "\n")
    else:
        file.write(tempVar)
